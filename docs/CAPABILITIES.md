...
| Project image and screenshots | Canonical fallback capture set complete | Twelve real browser/Android captures cover the public site, validated deterministic preview, deployed audit timeline, signed responder claim/lease/resolution and signed Android release Drill. Signed paths are redacted in provenance | Repeat the preview/trace captures after AgentCore is available, then compose the final project image |
...
9. Repeat the canonical preview/trace capture after AgentCore is restored; the deterministic fallback and all other live capture surfaces are complete.
