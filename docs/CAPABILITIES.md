...
| Plans | Hackathon demo live; production partially verified | Create/list/get/activate/pause/resume/test routes and Android/web clients; the Android judge entry mints an isolated session and routes its repository through the deployed demo API; both web and signed Android builds created a real draft and started an accelerated Drill | Add the live model compile leg after AgentCore access is restored |
...
| Public judge demo | Demo-only; live verified without the model leg | Fresh isolated sessions, draft creation and a complete deterministic Drill succeed through the canonical web demo and signed Android release; both clients use the deployed handlers with no browser fixtures or local-data fallback; demo device registration is impossible | Add the live model compile leg after AgentCore access is restored |
...
| Architecture artifact | Uploaded and verified | Nova-labelled 2400x1600 PNG, SVG source and visually checked one-page PDF; PNG and PDF are saved in the submitted Devpost entry | No remaining hackathon action |
| Project image and screenshots | Canonical submission package complete | Twelve real browser/Android captures cover the public site, validated deterministic preview, deployed audit timeline, signed responder claim/lease/resolution and signed Android release Drill. Signed paths are redacted in provenance. The final 1800×1200 GPT Image 2 composition preserves the exact ICO mark, a reviewed Higgsfield story frame and the real signed-Android drill. All 12 evidence screenshots are uploaded to the public Devpost gallery with captions | Optional score improvement: repeat the preview/trace captures after AgentCore is available |
| Demo video | Public and verified | [The 2:17 public YouTube demo](https://youtu.be/hDcBwr9dFsY) uses the conversational male Marcus preset through Higgsfield Qwen Audio 3.0 TTS Flash, an original project-authored score, a recorded public judge flow, recorded Android emulator proof, project architecture, 15 published timed English captions, a 1280×720 GPT Image 2 thumbnail, an editable FFmpeg build, and complete local provenance. YouTube reported no copyright or Community Guidelines issues. The narration and visible UI truthfully disclose the AgentCore quota block and deterministic fallback | No remaining hackathon action |
| Bonus posts | Public and submitted | Three Builder Center posts cover the product principle, governed AgentCore and idempotent workflow; all titles include “Agents for Humans.” Their verified short content URLs fit together in Devpost's single 255-character bonus field | No remaining hackathon action |
...
- Submission media: twelve canonical browser and signed-Android still captures, a 1800×1200 GPT Image 2 project image, and the published, visually reviewed 2:17 1920×1080 master are present. The film uses recorded live product motion, the male Marcus preset through Higgsfield Qwen Audio 3.0 TTS Flash, an original score, 15 published timed English captions, a 1280×720 GPT Image 2 thumbnail, an editable FFmpeg build, and complete asset/capture hashes. It contains no generated character footage or stock footage. The canonical deterministic fallback is explicitly disclosed in the UI and narration. Three optional bonus posts are public on AWS Builder Center and all three short content URLs are saved in the submitted Devpost entry.
...
- AWS core: the deployed API, workflow, queues and preserved AgentCore v1 resources remain available. The stack currently reports `UPDATE_ROLLBACK_COMPLETE` because the 2026-09-08 AWS edge attempt was rejected during certificate/CloudFront creation and rolled back; the canonical GitHub Pages site and API both remain HTTP 200. The API exposes all 61 explicit routes and uses one source-scoped invocation permission.
- AWS quota evidence: active AgentCore sessions were restored via approved request `451f1b8fde074b51bcb3aacaa2042ba8vNxnmcUj`. Request `b38dff125c3e4b1493e58c7fca4ed88bEgBdMI37` remains `CASE_OPENED`; a live 2026-09-09 Service Quotas read confirms the applied `Versions per Agent` value is still zero. Case `178851871600399` was answered on 2026-09-08 clarifying that the demo needs only the minimum non-zero value of one.
- AWS account-verification evidence: support case `178838741100092` was refreshed on 2026-09-08 with the CloudFront, Nova 2 Lite and AgentCore failures plus the September 14 deadline. On 2026-09-09, the Basic-plan Support API could not return case details and the console operator lacked `support:DescribeCases`; however, a direct Nova 2 Lite Converse canary still returned `ValidationException: Operation not allowed`, and CloudFormation still exposes `EdgeHostingStatus=BLOCKED_ACCOUNT_VERIFICATION`.
- Fresh AgentCore canary: on 2026-09-09, runtime `ico_demo_compiler-6NwvbhBK5L` version 1 and endpoint `DEFAULT` both reported `READY`, but invocation failed with `Runtime initialization time exceeded`. CloudWatch confirmed the recorded defect: `ModuleNotFoundError: No module named 'services.handlers'`. The corrected Nova artifact remains undeployable until the version quota becomes non-zero.
...
- Fresh canonical regression Drill: on 2026-09-09, `scripts/verify-live-demo-drill.sh` ran through `https://api.incaof.com`, created synthetic plan `31b37e9e-c9b5-4776-9d64-94fe6f90dd2a`, materialized Moment `dd99675a-fb41-5e10-a82a-dcbe403a7a20`, and resolved Alert `7f957125-ed08-466b-8f00-c314bf35847d`. Eleven audit events reached `RESPONDER_VERIFIED`; every provider reference remained under `safe-sink:` and tokens were not printed or stored.
